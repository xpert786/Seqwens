...
const AddPaymentMethodModal = ({ isOpen, onClose, onSuccess, isLoading = false }) => {
  ...
  return (
    <div
      className="fixed inset-0 bg-black/60 flex items-start justify-center z-[99999] p-2 sm:p-4 overflow-y-auto"
      onClick={handleClose}
    >
      <div
      ...
      >
        {/* Header */}
        <div className="flex justify-between items-center p-4 sm:p-5 border-b border-[#F1F5F9] bg-white sticky top-0 z-10">
          <div>
        ...
        {/* Footer */}
        <div className="flex flex-row justify-end gap-3 p-3 sm:p-5 border-t border-[#F1F5F9] bg-gray-50 sticky bottom-0 z-10">
          <button
            type="button"
            onClick={handleClose}
            disabled={loading || isLoading}
