...
export default function RevenueAnalysis({ activeTab, setActiveTab, tabs, period = '6m' }) {
  ...
  const kpiData = analyticsData?.kpis ? [
    {
      ...
      subtitle: analyticsData.kpis.gross_revenue?.subtitle || '',
      icon: (
    ...
    },
    {
      ...
      value: analyticsData.kpis.refund_transfers?.formatted || '$0',
      subtitle: analyticsData.kpis.refund_transfers?.subtitle || analyticsData.kpis.refund_transfers?.bank_adoption_formatted || '',
      icon: (
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  const revenueTrendData = analyticsData?.revenue_profit_trend?.map(item => ({
    ...
    refundTransfer: item.refund_transfer || 0,
    bankAdoptionPercentage: item.bank_adoption_percentage || 0
  })) || [];
