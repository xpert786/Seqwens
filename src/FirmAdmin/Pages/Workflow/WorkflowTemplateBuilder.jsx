      ...
      <div className="w-full max-w-5xl mx-auto">
        ...
        <div className="bg-white !rounded-lg !border border-[#E8F0FF] p-4 sm:p-5 mb-4 sm:mb-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <h4 className="text-xl sm:text-2xl font-bold text-gray-900 font-[BasisGrotesquePro]">
        ...
        {/* Quick Add Stages */}
        <div className="bg-white !rounded-lg !border border-[#E8F0FF] p-4 sm:p-5 mb-4 sm:mb-6">
          <h3 className="text-lg font-bold text-gray-900 mb-1 font-[BasisGrotesquePro]">
            Quick Add Stages
          </h3>
          <p className="text-sm text-gray-500 mb-4 font-[BasisGrotesquePro]">
            Tap any stage below to add it
          </p>
            ...
            {COMMON_STAGES.map((stage, idx) => {
              ...
              return (
                <button
                  ...
                  disabled={isAdded}
                  className={`p-3 text-left !rounded-lg !border transition-all flex flex-col justify-between h-full ${isAdded
                ...
                >
                  <div>
                    <div className="flex items-center gap-2 mb-2">
                      <span className="text-gray-400 flex-shrink-0">{stage.icon}</span>
                      <div className="flex-1 min-w-0">
                        <div className="font-semibold text-xs sm:text-sm text-gray-900 font-[BasisGrotesquePro] truncate">
                          {stage.name}
                        </div>
                      </div>
                    </div>
                    <div className="flex flex-wrap items-center gap-1.5 mt-1 ml-0 sm:ml-7">
                      <span className={`text-[10px] sm:text-xs px-2 py-0.5 rounded-full inline-flex items-center gap-1 ${userInfo.color}`}>
                        {userInfo.icon} {userInfo.label}
                      </span>
                      <span className="text-[10px] sm:text-xs text-gray-500 bg-gray-100 px-1.5 py-0.5 rounded">{stage.days}d</span>
                    </div>
                  </div>
                  {isAdded && (
                    <div className="text-[10px] text-green-600 font-bold mt-2 text-right">✓ Added</div>
                  )}
                </button>
              );
            })}
        ...
        {/* Current Stages */}
        <div className="bg-white !rounded-lg !border border-[#E8F0FF] p-4 sm:p-5">
          <h3 className="text-lg font-bold text-gray-900 mb-1 font-[BasisGrotesquePro]">
            Your Workflow
          </h3>
          <p className="text-sm text-gray-500 mb-4 font-[BasisGrotesquePro]">
            {stages.length} {stages.length === 1 ? 'stage' : 'stages'} added.{' '}
            {stages.length === 0
            ...
            <div className="space-y-3">
              {stages.map((stage, index) => {
                ...
                return (
                  <div
                    key={stage.id}
                    className="!border border-[#E8F0FF] !rounded-xl p-3 sm:p-4 bg-gradient-to-br from-white to-gray-50 shadow-sm"
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex flex-col items-center gap-1 sm:gap-2">
                        <button
                          ...
                          disabled={index === 0}
                          className="p-1 text-gray-400 hover:text-gray-600 disabled:opacity-30 touch-manipulation transition-colors"
                        >
                          <svg className="w-5 h-5 sm:w-4 sm:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 15l7-7 7 7" />
                          </svg>
                        </button>
                        <div className="w-6 h-6 sm:w-8 sm:h-8 flex items-center justify-center bg-[#F56D2D] text-white rounded-full text-xs sm:text-sm font-bold shadow-sm">
                          {index + 1}
                        </div>
                        <button
                      ...
                      <div className="flex-1 space-y-3">
                        <div className="flex items-center justify-between gap-2">
                          <input
                            ...
                            className="flex-1 text-sm sm:text-base font-semibold text-gray-900 bg-transparent border-b border-transparent hover:border-[#E8F0FF] focus:border-[#3AD6F2] focus:outline-none px-1 sm:px-2 py-1 font-[BasisGrotesquePro]"
                          />
                          {/* Mobile Delete button */}
                          <button
                            onClick={() => deleteStage(stage.id)}
                            className="sm:hidden p-1.5 text-red-400 hover:text-red-600"
                            title="Delete stage"
                          >
                      ...
                      <button
                        onClick={() => deleteStage(stage.id)}
                        className="hidden sm:block p-2 text-red-400 hover:text-red-600 hover:bg-red-50 !rounded-lg"
                        title="Delete stage"
                      >
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
