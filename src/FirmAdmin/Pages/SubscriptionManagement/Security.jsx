...
import AuditLogs from './SecurityTAll-tab/AuditLogs';
import SecurityPolicies from './SecurityTAll-tab/SecurityPolicies';
import PurchaseRestrictions from './SecurityTAll-tab/PurchaseRestrictions';

const Security = () => {
    const [activeSubTab, setActiveSubTab] = useState('PCI Compliance');

    const subTabs = ['PCI Compliance', 'Audit Logs', 'Security Polices', 'Purchase Restrictions'];
    ...
    return (
        <div className="bg-white !rounded-lg !border border-[#E8F0FF] p-4 sm:p-6">
            ...
            {/* Sub-tab Content */}
            {activeSubTab === 'PCI Compliance' && (
                <>
                    {/* Security Status Indicators - Only show in PCI Compliance */}
            ...
            {activeSubTab === 'Audit Logs' && <AuditLogs />}
            {activeSubTab === 'Security Polices' && <SecurityPolicies />}
            {activeSubTab === 'Purchase Restrictions' && <PurchaseRestrictions />}
        </div>
    );
};
