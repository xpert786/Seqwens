                          ...
                          <tr key={doc.id} style={{ cursor: "pointer" }}>
                            <td style={{ fontFamily: "BasisGrotesquePro", fontSize: "14px" }}>
                              <div className="d-flex align-items-center gap-2">
                                <FileIcon />
                                {doc.file_name || doc.name || doc.title || 'Untitled'}
                              </div>
                            </td>
                            <td style={{ fontFamily: "BasisGrotesquePro", fontSize: "14px" }}>{doc.file_type || 'N/A'}</td>
                            <td style={{ fontFamily: "BasisGrotesquePro", fontSize: "14px" }}>
                              {doc.file_size_formatted || (doc.file_size_bytes ? `${(doc.file_size_bytes / 1024).toFixed(2)} KB` : (doc.file_size ? `${(doc.file_size * 1024).toFixed(2)} KB` : '0.0 KB'))}
                            </td>
                            <td style={{ fontFamily: "BasisGrotesquePro", fontSize: "14px" }}>
                              {doc.updated_at_formatted || (doc.updated_at ? new Date(doc.updated_at).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' }) : 'N/A')}
                            </td>
                          </tr>
                        ))}
                      </tbody>
