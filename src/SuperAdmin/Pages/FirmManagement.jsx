...
export default function FirmManagement() {
    ...
    // Ref for the floating dropdown portal
    ...
    const fetchFirms = async () => {
        try {
            ...
            const response = await superAdminAPI.getFirms(
                ...
                planFilter === "All Plans" ? '' : planFilter.toLowerCase()
            );
    ...
    const getPlanColor = (plan, firm = null) => {
        if (firm && firm.is_billing_bypass) return 'bg-purple-600';
        const colors = {
            'starter': 'bg-[#FBBF24]',
            'team': 'bg-[#22C55E]',
            'growth': 'bg-[#22C55E]',
    ...
    const formatRevenue = (value) => {
        if (value === null || value === undefined || value === 0 || Number(value) === 0) return 'None';
    ...
    const handleViewAllFirmCards = (e) => {
        ...
        if (showAllFirmCards) setFirmCardsCurrentPage(1);
    };

    const handleFirmCardsPageChange = (newPage) => setFirmCardsCurrentPage(newPage);

    const handleExportReport = async () => {
        if (!pdfRef.current) {
            toast.warning('No data available to export.', { position: "top-right", autoClose: 3000 });
            return;
        }
        if (firms.length === 0) {
            toast.warning('No firms available to export.', { position: "top-right", autoClose: 3000 });
            return;
        }
        try {
            ...
            toast.error('Failed to generate PDF. Please try again.', { position: "top-right", autoClose: 3000 });
        }
    };

    // ── FIXED: toggle dropdown with fixed-position calculation ──────────────
    const toggleDropdown = (firmId, event) => {
        if (activeDropdown === firmId) {
            setActiveDropdown(null);
            return;
        }
        const btn = event.currentTarget;
        const rect = btn.getBoundingClientRect();
        const DROPDOWN_WIDTH = 192; // min-w-[12rem] = 192px
        const DROPDOWN_HEIGHT = 230; // approximate
        const viewportW = window.innerWidth;
        const viewportH = window.innerHeight;

        // Prefer right-aligned; flip left if overflowing
        ...
        if (top + DROPDOWN_HEIGHT > viewportH - 8) top = rect.top - DROPDOWN_HEIGHT - 6;

        setDropdownPos({ top, left });
        setActiveDropdown(firmId);
    };
    ...
    const suspendFirm = async () => {
        ...
        try {
            setSuspendingFirm(true); setSuspendError(null);
            const response = await superAdminAPI.suspendFirm(firmToSuspend.id, suspendReason);
            if (response.success) {
                setSuspendSuccess(true);
                await fetchFirms();
                setTimeout(() => {
                    setShowSuspendModal(false); setSuspendSuccess(false);
                    setSuspendReason(''); setFirmToSuspend(null);
                }, 2000);
            } else { throw new Error(response.message || 'Failed to suspend firm'); }
        } catch (err) { console.error(err); setSuspendError(handleAPIError(err)); }
        finally { setSuspendingFirm(false); }
    };

    const deleteFirm = async () => {
        if (!firmToDelete) return;
        try {
            setDeletingFirm(true); setDeleteError(null);
            const response = await superAdminAPI.deleteFirm(firmToDelete.id);
            if (response.success) {
                toast.success(response.message || 'Firm deleted successfully', { position: "top-right", autoClose: 3000 });
                setShowDeleteModal(false); setFirmToDelete(null);
                await fetchFirms();
            } else { throw new Error(response.message || 'Failed to delete firm'); }
        } catch (err) {
            console.error(err); setDeleteError(handleAPIError(err));
            toast.error(handleAPIError(err), { position: "top-right", autoClose: 3000 });
        } finally { setDeletingFirm(false); }
    };

    const unsuspendFirm = async () => {
        try {
            setUnsuspendingFirm(true); setUnsuspendError(null);
            const response = await superAdminAPI.reactivateFirm(firmToUnsuspend.id, unsuspendReason);
            if (response.success) {
                setUnsuspendSuccess(true);
                await fetchFirms();
                setTimeout(() => {
                    setShowUnsuspendModal(false); setUnsuspendSuccess(false);
                    setUnsuspendReason(''); setFirmToUnsuspend(null);
                }, 2000);
            } else { throw new Error(response.message || 'Failed to unsuspend firm'); }
        } catch (err) { console.error(err); setUnsuspendError(handleAPIError(err)); }
        finally { setUnsuspendingFirm(false); }
    };

    const loadUnassignedTaxpayers = async (page = 1) => {
        try {
            setLoadingAssignClients(true); setAssignClientError(null);
            const response = await superAdminAPI.getUnassignedTaxpayers(page, 20, '');
            if (response.success && response.data) {
                const taxpayers = response.data.taxpayers || [];
                const formattedOptions = taxpayers.map((t) => ({
                    id: t.id,
                    label: t.full_name || `${t.first_name || ''} ${t.last_name || ''}`.trim() || t.email || `Taxpayer ${t.id}`,
                    email: t.email,
                }));
                setAssignClientOptions(formattedOptions);
                setSelectedClientId((prev) => {
                    if (prev && formattedOptions.some((o) => o.id?.toString() === prev.toString()))
                        return prev.toString();
                    return formattedOptions[0]?.id?.toString() || '';
                });
                setAssignClientsSummary(response.data.summary || null);
            } else { throw new Error(response.message || 'Failed to load unassigned taxpayers'); }
        } catch (error) {
            console.error(error); setAssignClientError(handleAPIError(error));
            setAssignClientOptions([]); setSelectedClientId('');
        } finally { setLoadingAssignClients(false); }
    };

    const openAssignClientsModal = (firm) => {
        setAssignClientsFirm(firm); setAssignClientError(null);
        setAssignClientSuccess(false); setShowAssignClientsModal(true);
        loadUnassignedTaxpayers();
    };

    const closeAssignClientsModal = () => {
        setShowAssignClientsModal(false); setAssignClientsFirm(null);
        setAssignClientOptions([]); setSelectedClientId('');
        setAssignClientsSummary(null); setAssignClientError(null);
        setAssignClientSuccess(false); setAssigningClient(false);
        setLoadingAssignClients(false);
    };

    const handleAssignClient = async () => {
        if (!selectedClientId) { setAssignClientError('Please select a taxpayer to assign.'); return; }
        try {
            setAssigningClient(true); setAssignClientError(null);
            const firmId = assignClientsFirm?.id || assignClientsFirm?.firm_id;
            if (!firmId) throw new Error('Firm information missing. Please reopen the modal.');
            const response = await superAdminAPI.assignTaxpayerToFirm({
                taxpayerId: Number(selectedClientId), firmId: Number(firmId),
            });
            if (response.success) {
                setAssignClientSuccess(true);
                await fetchFirms(); await loadUnassignedTaxpayers();
                setTimeout(() => closeAssignClientsModal(), 1200);
            } else { throw new Error(response.message || 'Failed to assign client.'); }
        } catch (error) { console.error(error); setAssignClientError(handleAPIError(error)); }
    ...
    const handleAction = (action, firmId) => {
        ...
        if (action === 'View Details') {
            ...
            setFirmToSuspend(firm); setShowSuspendModal(true);
        } else if (action === 'Unsuspend Firm' || action === 'Reactivate Firm') {
            const firm = firms.find(f => f.id === firmId);
            setFirmToUnsuspend(firm); setShowUnsuspendModal(true);
        } else if (action === 'Assign Clients') {
            const firm = firms.find(f => f.id === firmId);
            openAssignClientsModal(firm);
        } else if (action === 'Delete') {
            const firm = firms.find(f => f.id === firmId);
            setFirmToDelete(firm); setShowDeleteModal(true);
        }
    };

    const handleAddFirm = () => setShowAddFirmModal(true);

    const handleCloseModal = () => {
        setShowAddFirmModal(false);
        setNewFirm({ firmName: "", ownerName: "", email: "", phone: "" });
    };
    ...
    const handleCreateFirm = async () => {
        ...
        try {
            setCreatingFirm(true); setCreateError(null);
            const firmData = {
                firm_name: newFirm.firmName, owner_name: newFirm.ownerName,
                email: newFirm.email, phone_number: newFirm.phone
            };
            const response = await superAdminAPI.createFirm(firmData);
            if (response.success) {
                setCreateSuccess(true);
                await fetchFirms();
                setTimeout(() => { handleCloseModal(); setCreateSuccess(false); }, 2000);
            } else { throw new Error(response.message || 'Failed to create firm'); }
        } catch (err) { console.error(err); setCreateError(handleAPIError(err)); }
        finally { setCreatingFirm(false); }
    };

    const closeFirmDetailsModal = () => {
        setShowFirmDetailsModal(false); setSelectedFirm(null); setFirmDetailsError(null);
    };
        ...
        <div className="min-h-screen bg-[rgb(243,247,255)] dark:bg-gray-900 lg:px-4 md:px-2 sm-px-1 lg:py-6 sm:py-2 md:px-6 transition-colors duration-200">
            ...
            <div ref={pdfRef} style={{ position: 'absolute', left: '-9999px', top: 0, width: '210mm', padding: '20mm', backgroundColor: 'white' }}>
                <div style={{ marginBottom: '20px' }}>
                    <h1 style={{ fontSize: '24px', fontWeight: 'bold', color: '#3B4A66', marginBottom: '10px' }}>Firms Report</h1>
                    <p style={{ fontSize: '12px', color: '#6B7280', marginBottom: '5px' }}>
                        Generated on: {new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}
                    </p>
                    <p style={{ fontSize: '12px', color: '#6B7280' }}>Total Firms: {firms.length}</p>
                </div>
                <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: '20px' }}>
                    <thead>
                        <tr style={{ backgroundColor: '#F3F4F6', borderBottom: '2px solid #E5E7EB' }}>
                            {['Firm', 'Plan', 'Status', 'Staff', 'Revenue', 'Created At'].map(h => (
                                <th key={h} style={{ padding: '10px', textAlign: 'left', fontSize: '12px', fontWeight: 'bold', color: '#3B4A66', border: '1px solid #E5E7EB' }}>{h}</th>
                            ))}
                        </tr>
                    </thead>
                    <tbody>
                        {firms.map((firm, index) => (
                            <tr key={`${firm.id}-${index}`} style={{ borderBottom: '1px solid #E5E7EB' }}>
                                ...
                                <td style={{ padding: '10px', fontSize: '11px', color: '#3B4A66', border: '1px solid #E5E7EB' }}>{formatPlan(firm)}</td>
                                <td style={{ padding: '10px', fontSize: '11px', color: '#3B4A66', border: '1px solid #E5E7EB' }}>{formatStatus(firm.status)}</td>
                                <td style={{ padding: '10px', fontSize: '11px', color: '#3B4A66', border: '1px solid #E5E7EB' }}>{firm.staff_count ?? 0}</td>
                                <td style={{ padding: '10px', fontSize: '11px', color: '#3B4A66', border: '1px solid #E5E7EB' }}>{formatRevenue(firm.total_revenue)}</td>
                                <td style={{ padding: '10px', fontSize: '11px', color: '#3B4A66', border: '1px solid #E5E7EB' }}>{formatDate(firm.created_at) || '—'}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
            ...
            {/* ── FLOATING DROPDOWN PORTAL ─────────────────────────────────────────
                Rendered at the document root level (via fixed positioning) so it is
                never clipped by overflow:hidden / overflow:auto ancestors.            */}
            {activeDropdown !== null && (() => {
                const firm = firms.find(f => f.id === activeDropdown);
                if (!firm) return null;
                ...
                return (
                    <div
                        ...
                        className="bg-white dark:bg-gray-800 rounded-lg border border-[#E8F0FF] dark:border-gray-700 py-2 shadow-xl"
                    >
                        {/* View Details */}
                        <button onClick={() => handleAction('View Details', firm.id)}
                            className="w-full px-4 py-2 text-left text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 flex items-center gap-2">
                            <svg width="16" height="16" viewBox="0 0 20 20" fill="none"><path d="M10 12.5C11.3807 12.5 12.5 11.3807 12.5 10C12.5 8.61929 11.3807 7.5 10 7.5C8.61929 7.5 7.5 8.61929 7.5 10C7.5 11.3807 8.61929 12.5 10 12.5Z" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /><path d="M2.04883 10C3.11049 6.61917 6.26716 4.16667 10.0005 4.16667C13.7338 4.16667 16.8905 6.61917 17.9522 10C16.8905 13.3808 13.7338 15.8333 10.0005 15.8333C6.26716 15.8333 3.11049 13.3808 2.04883 10Z" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /></svg>
                            View Details
                        </button>

                        {/* Login as Firm */}
                        <button onClick={() => handleAction('Login as Firm', firm.id)}
                            className="w-full px-4 py-2 text-left text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 flex items-center gap-2">
                            <svg width="16" height="16" viewBox="0 0 20 20" fill="none"><path d="M15.8333 9.16667V5.83333C15.8333 5.39131 15.6577 4.96738 15.345 4.65482C15.0325 4.34226 14.6085 4.16667 14.1667 4.16667H5.83333C5.39131 4.16667 4.96738 4.34226 4.65482 4.65482C4.34226 4.96738 4.16667 5.39131 4.16667 5.83333V14.1667C4.16667 14.6087 4.34226 15.0326 4.65482 15.3452C4.96738 15.6577 5.39131 15.8333 5.83333 15.8333H14.1667C14.6085 15.8333 15.0325 15.6577 15.345 15.3452C15.6577 15.0326 15.8333 14.6087 15.8333 14.1667V10.8333M12.5 10H18.3333M18.3333 10L16.25 7.91667M18.3333 10L16.25 12.0833" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /></svg>
                            Login as Firm
                        </button>
                        ...
                        <button
                            ...
                            className="w-full px-4 py-2 text-left text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 flex items-center gap-2">
                            <svg width="16" height="16" viewBox="0 0 20 20" fill="none"><path d="M10 1.66667V5M10 15V18.3333M3.57501 3.575L5.83334 5.83333M14.1667 14.1667L16.425 16.425M1.66667 10H5M15 10H18.3333M3.57501 16.425L5.83334 14.1667M14.1667 5.83333L16.425 3.575" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /></svg>
                            {status === 'suspended' ? 'Unsuspend Firm' : status === 'inactive' ? 'Reactivate Firm' : 'Suspend Firm'}
                        </button>

                        {/* Assign Clients */}
                        <button onClick={() => handleAction('Assign Clients', firm.id)}
                            className="w-full px-4 py-2 text-left text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 flex items-center gap-2">
                            <svg width="16" height="16" viewBox="0 0 20 20" fill="none"><path d="M13.3334 17.5V15.8333C13.3334 14.9493 12.9822 14.1014 12.357 13.4763C11.7319 12.8512 10.884 12.5 10.0001 12.5H4.16675C3.28269 12.5 2.43485 12.8512 1.80972 13.4763C1.1846 14.1014 0.833416 14.9493 0.833416 15.8333V17.5M16.6667 6.66667V11.6667M19.1667 9.16667H14.1667M10.4167 5.83333C10.4167 7.67428 8.9243 9.16667 7.08341 9.16667C5.24252 9.16667 3.75008 7.67428 3.75008 5.83333C3.75008 3.99238 5.24252 2.5 7.08341 2.5C8.9243 2.5 10.4167 3.99238 10.4167 5.83333Z" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /></svg>
                            Assign Clients
                        </button>
                        ...
                        {/* Delete */}
                        <button onClick={() => handleAction('Delete', firm.id)}
            ...
            <div className="mx-auto flex w-full flex-col gap-6 firmmgmt-page">
                ...
                <div className="rounded-lg mb-6">
                    <div className="flex flex-col lg:flex-row gap-2 firmmgmt-search-row">
                        <div className="relative w-full lg:w-[300px] firmmgmt-search">
                            ...
                            <input type="text" placeholder="Search by firm name or email"
                ...
                <div className="bg-white dark:bg-gray-800 rounded-lg border-1 border-[#E8F0FF] dark:border-gray-700 firmmgmt-table-card transition-all">
                    <div className="p-6">
                        <div className="flex justify-between items-start mb-2 firmmgmt-table-card-header">
                            <div>
                                <h4 className="text-md font-bold text-gray-800 dark:text-white mb-2">Firms</h4>
                                <p className="text-gray-500 dark:text-gray-400 text-sm">Comprehensive list of all firms registered on the platform</p>
                            </div>
                    ...
                    {/* ── Table: overflow-x-auto only; DO NOT set overflow-y ───────────── */}
                    <div className="overflow-x-auto">
                        <div className="min-w-[900px]">
                            {/* Table Headers */}
                            <div className="px-3 py-3 firmmgmt-table-header border-b border-[#E8F0FF] dark:border-gray-700">
                                <div className="grid grid-cols-12 gap-3 items-center text-sm font-semibold text-gray-600 dark:text-gray-400">
                                    <div className="col-span-3">Firm</div>
                                    <div className="col-span-2 lg:col-span-1">Plan</div>
                            ...
                            {!loading && (
                                <div className="divide-y divide-gray-200 firmmgmt-table-list">
                                    {displayedFirmCards.length > 0 ? displayedFirmCards.map((firm) => (
                                        <div key={firm.id}
                                            className="pr-1 pl-3 py-3 transition-colors border border-[#E8F0FF] dark:border-gray-700 m-2 firmmgmt-table-row bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700/50"
                                            style={{ borderRadius: '7px' }}>
                                            <div className="grid grid-cols-12 gap-4 items-center firmmgmt-table-row-grid">
                                                ...
                                                <div className="col-span-2 lg:col-span-1">
                                                    <span className={`inline-flex px-2 py-0.5 rounded-full text-[11px] font-medium text-white ${getPlanColor(firm.subscription_plan, firm)}`}>
                                                        {formatPlan(firm)}
                                                    </span>
                                                </div>
                                                ...
                                                {/* Status */}
                                                <div className="col-span-2 lg:col-span-1">
                                                    <span className={`inline-flex px-2 py-0.5 rounded-full text-[11px] font-medium text-white ${getStatusColor(firm.status)}`}>
                                                ...
                                                {/* Actions — trigger button only; menu is rendered in the portal above */}
                                                <div className="col-span-1 flex justify-center">
                                                    <button
                                                        data-dropdown-trigger="true"
                                                        onClick={(e) => toggleDropdown(firm.id, e)}
                    ...
                    {shouldShowFirmCardsPagination && (
                        <div className="flex items-center justify-between px-4 py-3 border-t border-[#E8F0FF] dark:border-gray-700 firmmgmt-pagination bg-white dark:bg-gray-800 rounded-b-lg">
                            <button onClick={() => handleFirmCardsPageChange(firmCardsCurrentPage - 1)}
                                    ...
                                    <>
                                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                                            <div className="bg-white dark:bg-gray-800 border border-[#E8F0FF] dark:border-gray-700 rounded-xl p-4">
                                                ...
                                                <div className="text-sm text-gray-600 dark:text-gray-300">
                                                    Monthly Fee: <span className="font-semibold text-gray-900 dark:text-white">{formatCurrency(selectedFirm.firm?.subscription_plan?.monthly_fee)}</span>
                                                </div>
                                            </div>
                                            <div className="bg-white dark:bg-gray-800 border border-[#E8F0FF] dark:border-gray-700 rounded-xl p-4">
                                                ...
                                                <p className="text-sm text-gray-500 dark:text-gray-400 mt-2">Joined on {formatDate(selectedFirm.firm?.join_date)}</p>
                                            </div>
                                            <div className="bg-white dark:bg-gray-800 border border-[#E8F0FF] dark:border-gray-700 rounded-xl p-4">
                                                <p className="text-sm text-gray-500 dark:text-gray-400 mb-1">Last Active</p>
                                                <h4 className="text-lg font-semibold text-gray-900 dark:text-white">{formatDateTime(selectedFirm.system_health?.last_active)}</h4>
                                                <p className="text-sm text-gray-500 dark:text-gray-400 mt-2">
                                                    Overall Health: <span className="font-semibold">{selectedFirm.system_health?.overall_health?.status || 'N/A'} ({selectedFirm.system_health?.overall_health?.score ?? '—'}%)</span>
                                                </p>
                                            </div>
                                        </div>
                                        ...
                                        <div className="bg-white dark:bg-gray-800 border border-[#E8F0FF] dark:border-gray-700 rounded-xl p-5">
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                                                <div><p className="text-gray-500 dark:text-gray-400">Firm Email</p><p className="font-medium text-gray-900 dark:text-white">{selectedFirm.firm?.firm_email || 'N/A'}</p></div>
                                                <div><p className="text-gray-500 dark:text-gray-400">Phone</p><p className="font-medium text-gray-900 dark:text-white">{selectedFirm.firm?.phone || 'N/A'}</p></div>
                                                <div>
                                                ...
                                                <div><p className="text-gray-500 dark:text-gray-400">Tax ID</p><p className="font-medium text-gray-900 dark:text-white">{selectedFirm.firm?.tax_id || 'N/A'}</p></div>
                                            </div>
                                            <div className="mt-4">
                                                <p className="text-gray-500 dark:text-gray-400 text-sm mb-1">Address</p>
                                                <p className="text-sm font-medium text-gray-900 dark:text-white">
                                                    {[selectedFirm.firm?.address?.street, selectedFirm.firm?.address?.city, selectedFirm.firm?.address?.state, selectedFirm.firm?.address?.zip_code, selectedFirm.firm?.address?.country].filter(Boolean).join(', ') || 'N/A'}
                                                </p>
                                            </div>
                                        </div>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                            <div className="bg-white dark:bg-gray-800 border border-[#E8F0FF] dark:border-gray-700 rounded-xl p-4">
                                                ...
                                                <h3 className="text-2xl font-semibold text-gray-900 dark:text-white mt-1">{selectedFirm.metrics?.users?.count ?? '—'}</h3>
                                            </div>
                                            <div className="bg-white dark:bg-gray-800 border border-[#E8F0FF] dark:border-gray-700 rounded-xl p-4">
                                                <p className="text-sm text-gray-500 dark:text-gray-400">{selectedFirm.metrics?.clients?.label || 'Clients'}</p>
                                                <h3 className="text-2xl font-semibold text-gray-900 dark:text-white mt-1">{selectedFirm.metrics?.clients?.count ?? '—'}</h3>
                                            </div>
                                            <div className="bg-white dark:bg-gray-800 border border-[#E8F0FF] dark:border-gray-700 rounded-xl p-4">
                                                <p className="text-sm text-gray-500 dark:text-gray-400">Monthly Subscription</p>
                                                <h3 className="text-xl font-semibold text-gray-900 dark:text-white mt-1">{formatCurrency(selectedFirm.metrics?.revenue?.monthly_subscription)}</h3>
                                                <p className="text-sm text-gray-500 dark:text-gray-400 mt-2">Paid Invoice Revenue: <span className="font-semibold text-gray-900 dark:text-white">{formatCurrency(selectedFirm.metrics?.revenue?.paid_invoice_revenue)}</span></p>
                                            </div>
                                        </div>
                                        ...
                                        <div className="bg-white dark:bg-gray-800 border border-[#E8F0FF] dark:border-gray-700 rounded-xl p-5">
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                <div>
                                                    ...
                                                    <p className="text-2xl font-bold text-gray-900 dark:text-white mt-1">{selectedFirm.system_health?.overall_health?.score ?? '—'}</p>
                                                    <p className="text-xs text-gray-400 mt-1 italic">Includes staff and clients</p>
                                                </div>
                                                <div>
                                                    <p className="text-sm text-gray-500 dark:text-gray-400">Storage Usage</p>
                                                    <p className="text-xl font-semibold text-gray-900 dark:text-white mt-1">{selectedFirm.system_health?.storage?.used_gb ?? '—'} GB / {selectedFirm.system_health?.storage?.limit_gb ?? '—'} GB</p>
                                                    <div className="mt-2">
                                                        <div className="w-full bg-gray-200 dark:bg-gray-600 rounded-full h-2">
                                                            <div className="bg-orange-500 h-2 rounded-full" style={{ width: `${Math.min(selectedFirm.system_health?.storage?.percent_used ?? 0, 100)}%` }}></div>
                                                        </div>
                                                        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{selectedFirm.system_health?.storage?.percent_used ?? '—'}% used</p>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                    ...
                    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
                        ...
                        <div className="relative bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-xl mx-auto transition-all flex flex-col max-h-[90vh] overflow-hidden" style={{ borderRadius: '12px' }} onClick={(e) => e.stopPropagation()}>
                            ...
                            <div className="p-6 space-y-4 overflow-y-auto max-h-[min(65vh,500px)]">
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Firm Name</label>
                                    <input type="text" value={newFirm.firmName} onChange={(e) => handleInputChange('firmName', e.target.value)} placeholder="Enter Firm name"
                                ...
                                <div>
                                    ...
                                    <input type="text" value={newFirm.ownerName} onChange={(e) => handleInputChange('ownerName', e.target.value)} placeholder="Enter Owner Name"
                                        className="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400" />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Email Address</label>
                                    <input type="email" value={newFirm.email} onChange={(e) => handleInputChange('email', e.target.value)} placeholder="Enter Email Address"
                                        className="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400" />
                                </div>
                                <div className="relative">
                                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Phone Number</label>
                                    <PhoneInput country={phoneCountry} value={newFirm.phone || ''}
                                        onChange={(phone) => handleInputChange('phone', phone)}
                                        onCountryChange={(countryCode) => setPhoneCountry(countryCode.toLowerCase())}
                                        enableSearch countryCodeEditable={false} specialLabel=""
                                ...
                                <div className="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-3">
                                    ...
                                    <div className="text-sm text-gray-600 dark:text-gray-300">
                                        ...
                                        <div><strong>Current Status:</strong>
                                            <span className={`ml-1 px-2 py-0.5 rounded-full text-sm font-medium text-white ${getStatusColor(firmToSuspend.status)}`}>{formatStatus(firmToSuspend.status)}</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="bg-yellow-50 dark:bg-yellow-900/10 border border-yellow-200 dark:border-yellow-900/30 rounded-lg p-3">
                                    <div className="flex">
                                        ...
                                        <div className="ml-3">
                                            ...
                                            <p className="mt-2 text-sm text-yellow-700 dark:text-yellow-600">This action will suspend the firm and all associated users. They will not be able to access the platform until reactivated.</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="p-4 space-y-4 overflow-y-auto max-h-[min(60vh,400px)]">
                                ...
                                {unsuspendSuccess && (
                                    <div className="bg-green-50 dark:bg-green-900/10 border border-green-200 dark:border-green-900/30 rounded-lg p-3">
                                        <div className="text-sm text-green-700 dark:text-green-400">Firm {firmToUnsuspend.status?.toLowerCase() === 'inactive' ? 'reactivated' : 'unsuspended'} successfully!</div>
                                    </div>
                                )}
                                ...
                                <div className="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-3">
                                    ...
                                    <div className="text-sm text-gray-600 dark:text-gray-300">
                                        ...
                                        <div><strong>Current Status:</strong>
                                            <span className={`ml-1 px-2 py-0.5 rounded-full text-sm font-medium text-white ${getStatusColor(firmToUnsuspend.status)}`}>{formatStatus(firmToUnsuspend.status)}</span>
                                        </div>
                                    </div>
                                </div>

                                <div>
                                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Reason / Notes (optional)</label>
                                    <textarea value={unsuspendReason} onChange={(e) => setUnsuspendReason(e.target.value)}
                                        placeholder="Add any notes about reactivating this firm..."
                                        className="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent resize-none bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400"
                                        rows={3} />
                                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">This note is stored with the audit log for future reference.</p>
                                </div>
                        ...
                        <div className="relative bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-xl mx-auto transition-all flex flex-col max-h-[90vh] overflow-hidden" style={{ borderRadius: '12px' }}>
                            ...
                            <div className="p-4 space-y-3 overflow-y-auto max-h-[min(60vh,400px)]">
                                ...
                                {assignClientsSummary?.total_unassigned !== undefined && (
                                    <p className="text-xs text-gray-500 dark:text-gray-400">{assignClientOptions.length} of {assignClientsSummary.total_unassigned} unassigned taxpayers shown</p>
                                )}
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Select Taxpayer</label>
                                    <select value={selectedClientId} onChange={(e) => setSelectedClientId(e.target.value)}
                                        ...
                                        {assignClientOptions.length > 0 ? (
                                            <>
                                                <option value="" disabled>Choose a taxpayer</option>
                                                {assignClientOptions.map((client) => (
                                                    <option key={client.id} value={client.id}>{client.label}</option>
                                                ))}
                                            </>
                                        ) : (
                                            <option value="">No taxpayers available for assignment</option>
                                        )}
                                    </select>
                                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Only taxpayers not yet assigned to any firm are listed.</p>
                                </div>
                            </div>
