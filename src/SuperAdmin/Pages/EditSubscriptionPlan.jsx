    ...
    if (response.ok) {
      ...
      if (result.success && result.data) {
        ...
        setLimits({
          ...
          eSignatures: planData.e_signatures_per_month || '0',
          includedOffices: parseInt(planData.included_offices) || 1,
          maxWorkflows: parseInt(planData.max_workflows) || 0
        });
...
const getFeatures = () => {
  ...
  if (storage !== '') {
    ...
    if (workflows !== '') {
      ...
      if (workflowsNum === 0 || workflows.toString().toLowerCase() === 'unlimited') {
                  ...
                  <div className='space-y-4 flex flex-row gap-4 w-fit edit-plan-inline'>
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          const v = e.target.value;
                          setLimits({ ...limits, maxWorkflows: v === '' ? '' : v });
                        }}
                        onBlur={(e) => {
                          const n = parseInt(e.target.value);
                          setLimits({ ...limits, maxWorkflows: isNaN(n) ? 0 : Math.max(0, n) });
                        }}
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                        style={{ border: '1px solid #E8F0FF', color: '#3B4A66' }}
                      />
