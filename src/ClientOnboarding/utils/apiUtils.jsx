...
export const dashboardAPI = {
  ...
  getOnboardingStatus: async () => {
    return await apiRequest('/taxpayer/onboarding-status/', 'GET');
  },
  // Update onboarding status
  updateOnboardingStatus: async (completed) => {
    return await apiRequest('/taxpayer/onboarding-status/', 'POST', { onboarding_completed: completed });
  },
...
export const firmAdminDocumentsAPI = {
  ...
  // Delete document
  // DELETE /firm/clients/:clientId/documents/:documentId/
  deleteDocument: async (clientId, documentId) => {
    ...
    const url = `${API_BASE_URL}/firm/clients/${clientId}/documents/${documentId}/`;
...
export const firmAdminClientsAPI = {
  ...
  updateClient: async (clientId, payload) => {
    try {
      ...
      return await (await makeRequest()).json();
    } catch (error) {
  ...
  softDeleteClient: async (clientId) => {
    return await apiRequest(`/firm/taxpayers/${clientId}/soft-delete/`, 'DELETE');
  },

  // Restore a soft-deleted client
  restoreClient: async (clientId) => {
    return await apiRequest(`/firm/taxpayers/${clientId}/restore/`, 'POST');
  },
...
export const documentsAPI = {
  ...
  // Browse documents for taxpayer (root or in a folder)
  // GET /taxpayer/my-documents/browse/
  browseDocuments: async (params = {}) => {
    const { folder_id, show_archived, search, category_id, sort_by, recursive } = params;
    const queryParams = new URLSearchParams();
    ...
    if (category_id) queryParams.append('category_id', category_id);
    if (sort_by) queryParams.append('sort_by', sort_by);
    if (recursive) queryParams.append('recursive', recursive);

    const queryString = queryParams.toString();
    const endpoint = `/taxpayer/my-documents/browse/${queryString ? `?${queryString}` : ''}`;
    return await apiRequest(endpoint, 'GET');
  },

  // Optimized folder browsing for taxpayer
  // GET /taxpayer/my-documents/folders/
  browseFoldersSplit: async (params = {}) => {
    const { folder_id, search } = params;
    const queryParams = new URLSearchParams();
    if (folder_id) queryParams.append('folder_id', folder_id);
    ...
    const queryString = queryParams.toString();
    const endpoint = `/taxpayer/my-documents/folders/${queryString ? `?${queryString}` : ''}`;
    return await apiRequest(endpoint, 'GET');
  },

  // Optimized file browsing for taxpayer
  // GET /taxpayer/my-documents/files/
  browseFilesSplit: async (params = {}) => {
    const { folder_id, search, show_archived, category_id, sort_by, page, page_size } = params;
    const queryParams = new URLSearchParams();
    if (folder_id) queryParams.append('folder_id', folder_id);
    ...
    const queryString = queryParams.toString();
    const endpoint = `/taxpayer/my-documents/files/${queryString ? `?${queryString}` : ''}`;
    return await apiRequest(endpoint, 'GET');
  },
...
export const firmAdminMessagingAPI = {
  ...
  getActiveUsers: async (params = {}) => {
    const { search, role, exclude_existing_chats } = params;
    const queryParams = new URLSearchParams();

    if (search) queryParams.append('search', search);
    if (role) queryParams.append('role', role);
    if (exclude_existing_chats) queryParams.append('exclude_existing_chats', 'true');
